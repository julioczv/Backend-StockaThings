...
public class SaleService {
    ...
    public SaleResponseDTO createSale(SaleRequestDTO in, User me) {
        ...
        // monta itens
        var saleItems = new ArrayList<SaleItemResponseDTO>();
        ...
        for (var item : in.items()) {
            ...
            // baixa no estoque (salva explicitamente)
            product.setQtdProduto(product.getQtdProduto() - qtd);
            productRepository.save(product);

            // resposta do item
            saleItems.add(new SaleItemResponseDTO(
                    si.getIdItemVenda(),
        ...
        // totais calculados a partir da lista (evita erro de size())
        int totalItens = saleItems.stream().mapToInt(SaleItemResponseDTO::quantidade).sum();
        BigDecimal totalVenda  = saleItems.stream()
                .map(SaleItemResponseDTO::subTotalVenda)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal totalCompra = saleItems.stream()
    ...
    public SalePeriodDTO findByDateRange(LocalDate from, LocalDate to) {
        ...
        BigDecimal custoPeriodo = saleDtos.stream().map(d -> d.totals().custo()).reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal faturadoPeriodo = saleDtos.stream().map(d -> d.totals().faturado()).reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        for (var si : itens) {
            ...
            if (rows == 0) {
                throw new ResponseStatusException(HttpStatus.CONFLICT,
                        "Não foi possível estornar o estoque do produto " + si.getProduto().getNomeProduto());
            }
        }
